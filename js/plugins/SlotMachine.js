...
// deleted by nekoma. ->start
//    Scene_MenuBase.prototype.createHelpWindow.call(this);
//    this._helpWindow.y = Graphics.boxHeight - this._helpWindow.height;
// deleted by nekoma. <-end
    this._helpWindow = new Window_SlotMachineHelp();
    this._helpWindow.setReplayCommand(this.replayCommand.bind(this));
...
Scene_SlotMachine.prototype.betCommand = function () {
    ...
    if (this._bet > 2) {
        this._slotCommandWindow.disableBet();
        this._slotCommandWindow.select(1);
    }
...
//        this._helpWindow.setText(lostMessage + '\n' + replayMessage); // deleted by nekoma.
        this._helpWindow.setText(lostMessage, true);    // added by nekoma.
        this._helpWindow._isReplay = true;              // added by nekoma.
        this._helpWindow.open();
...
//        this._helpWindow.setText(coinFullMessage + '\n' + replayMessage);
        this._helpWindow.setText(coinFullMessage, true);
    }
    else {
//        this._helpWindow.setText(this._winMessage + '\n' + replayMessage);
        this._helpWindow.setText(this._winMessage, true);
    }
    this._helpWindow._isReplay = true;      // added by nekoma.
    this._winMessage = "";
//    this._replayCommandWindow.open();     // deleted by nekoma.
//    this._replayCommandWindow.activate(); // deleted by nekoma.
};
...
// added by nekoma. ->start
        else if (this._slotCommandWindow.isAllowSpin && !this._spinStart) {
            SoundManager.playOk();
            this.spinCommand();
        }
// added by nekoma. <-end
...
function Window_SlotMachineHelp() {
    this.initialize.apply(this, arguments);
};

Window_SlotMachineHelp.prototype = Object.create(Window_Message.prototype);
Window_SlotMachineHelp.prototype.constructor = Window_SlotMachineHelp;

Window_SlotMachineHelp.prototype.initialize = function() {
    ...
    this._replayCommand = null;
    this._isReplay = false;
};
    
Window_SlotMachineHelp.prototype.update = function() {
    Window_Message.prototype.update.call(this);
};

Window_SlotMachineHelp.prototype.terminateMessage = function() {
    if (this._replayCommand && this._isReplay) {
        this._isReplay = false;
        this._replayCommand();
    }
};
  
Window_SlotMachineHelp.prototype.setReplayCommand = function(replayCommand) {
    this._replayCommand = replayCommand;
};
    
Window_SlotMachineHelp.prototype.setText = function(text, wait) {
    this._textState = {};
    this._textState.index = 0;
    this._textState.text = '\x1b>' + text + (wait ? '' : '\x1b^');
